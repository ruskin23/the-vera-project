...
def test_testkit_runs_green_on_simple_fixture(simple_challenge: Path, monkeypatch) -> None:
    ...
    assert report.ok, "\n".join(
        f"{line.ok}: {line.text} {line.details}" for line in report.lines
    )
...
def test_testkit_fails_when_solution_missing(simple_challenge: Path, monkeypatch) -> None:
    ...
    assert any(
        "fixtures/solution" in line.text for line in report.lines if not line.ok
    )


def test_testkit_fails_when_grader_passes_pristine(
    simple_challenge: Path, monkeypatch
) -> None:
